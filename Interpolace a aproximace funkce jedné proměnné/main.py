...
ax[0].plot(x, lr_p.predict(x.reshape(-1, 1)), label="Linear Regression")
ax[0].plot(x, polynomial(x, *popt_p), label="Quadratic Polynomial")
ax[0].plot(x, spline_p(x), label="Linear Spline")
ax[0].set_xlabel("x")
ax[0].set_ylabel("y")
...
ax[1].plot(x, lr_h.predict(x.reshape(-1, 1)), label="Linear Regression")
ax[1].plot(x, harmonic(x, *popt_h), label="Harmonic Function")
ax[1].plot(x, spline_h(x), label="Linear Spline")
ax[1].set_xlabel("x")
ax[1].set_ylabel("y")
...
ax[2].plot(x, lr_l.predict(np.log(x).reshape(-1, 1)), label="Linear Regression")
ax[2].plot(x, logarithm(x, *popt_l), label="Logarithmic Function")
ax[2].plot(x, spline_l(x), label="Linear Spline")
ax[2].set_xlabel("x")
ax[2].set_ylabel("y")
...
p_err = np.sum((y_p - lr_p.predict(x.reshape(-1, 1)))**2)
h_err = np.sum((y_h - lr_h.predict(x.reshape(-1, 1)))**2)
l_err = np.sum((y_l - lr_l.predict(np.log(x).reshape(-1, 1)))**2)

print("Sum of Squared Errors:")
print("Polynomial: ", p_err)
print("Harmonic: ", h_err)
print("Logarithmic: ", l_err)
